...
def create_action_folder(data_path, action):
    # loại bỏ hoặc thay thế ký tự không hợp lệ trong tên thư mục
...
with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5,model_complexity=0) as holistic:
    ...
    # Track index for each action to avoid overwriting files
    action_idx = {action: 0 for action in pd.unique(df['LABEL'])}
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc='Process actions'):
        ...
        augmenteds = generate_augmented_samples(frame_lists,augmentations,50,5)
        augmenteds.append(frame_lists)

        for aug in augmenteds:
          seq = interpolate_keypoints(aug)
           # Lưu .npz chứa sequence và label
          file_path = os.path.join(action_path, f'{action_idx[action]}.npz')
          np.savez(
              file_path,
              sequence=seq,
              label=label
        ...
        # Avoid Unicode encoding error on Windows console
        print(f"Action {action_position[action]}/{len(df['LABEL'].unique())} : Processed {action_idx[action]} samples - Time: {time.get_time()}")
